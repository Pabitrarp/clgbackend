...
exports.orderDetails = async (req, res) => {
  ...
  try {
    const orderDetail = await order_model
      .find({ user: uid })
...
exports.setAddress = async (req, res) => {
  const { user } = req.body; // Extracting user from request body
  try {
    // Assuming 'user' is an identifier like ID or email
    const userinfo = await order_model.findOne({ user: user }); // Using 'user' as the query object
    if (userinfo) {
      res.status(200).send({
        success: true,
        ...
        houseType: userinfo.houseType,
        mobile: userinfo.mobile,
      });
    } else {
      res
        .status(404)
        .send({ success: false, message: "User information not found" }); // Correcting status code to 404
    }
  } catch (err) {
    console.error(err); // Logging error to console
    res.status(500).send({ success: false, message: "Internal server error" }); // Sending 500 status code and error message
  }
};
